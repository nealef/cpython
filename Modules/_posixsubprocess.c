...
#else
/**
 * Use posix spawn to start a new process
 *
 * - create fd map that will keep open those we want
 *
 */

_Py_NO_INLINE static pid_t
child_spawn(char *const exec_array[],
           char *const argv[],
           char *const envp[],
           const char *cwd,
           int p2cread, int p2cwrite,
           ...
           PyObject *preexec_fn_args_tuple)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (fdMap[i] != -1) {
            flags = fcntl(fdMap[i], F_GETFD, flags);
            fcntl(fdMap[i], F_SETFD, (flags & ~FD_CLOEXEC));
        }
    }
    ...
    if (child_sigmask) {
        ...
        memcpy(&inherit.sigmask, child_sigmask, sizeof(inherit.sigmask));
    } else {
...
fprintf(stderr, "errpid: %d\n", pid);
    ...
    /* We ignore all write() return values as the total size of our writes is
       ...
       signal (fails with EINTR). */
    if (saved_errno) {
        char *err = __alloca(80);
        int l;
        l = sprintf(err, "OSError:%x:%s", saved_errno, (reached_preexec ? "" : "noexec"));
        _Py_write_noraise(errpipe_write, err, l);
    } else {
        _Py_write_noraise(errpipe_write, "SubprocessError:0:", 18);
        _Py_write_noraise(errpipe_write, err_msg, strlen(err_msg));
    }
